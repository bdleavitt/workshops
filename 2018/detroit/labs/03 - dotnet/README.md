...
* [Task 0: Remote Desktop to a Windows Node](#0)
...
## <a name="0"></a>Step 0: Remote Desktop to a Windows Node
...
From the [Azure Portal](https://portal.azure.com) open the blade for the first windows node, ex. **docker-ee-win-1**, and download the `.rdp` file. 
...
Once git is ready we can clone the repository of working files to the filesystem:
...
> Windows Server Container images are large. The inital pull of these images can take 15+ minutes
...
To see the app running, run `docker ps` from the command line to see the running containers. Identity the container that is using the `dockersamples/mta-dev-signup-app:v1` image.

![image](./images/psa.png)

Inspect the contact with `docker inspect <Container Name>`, example `docker inspect docker inspect app_signup-app_1_5ec7c4146b27`. 

![image](./images/dockerinspect.png)

Copy the IP address and open in Firefox to see the application.
...
```.term1
docker container exec app_signup-db_1_7eb4948690c8 powershell -Command "Invoke-SqlCmd -Query 'SELECT * FROM Prospects' -Database SignUpDb"
```

> Retrieve the container name for your database with the `docker ps` command, similar to before with the application container
...
The new website will be available on your same Windows Docker host. Browse to the Windows server as before - using the IP address from `docker inspect <application container name>`. You'll see the new homepage has been improved with an advert for DockerCon!
...
The DTR domain will be something like `dtrapp-dockeree2-alk7.eastus.cloudapp.azure.com`. To make it easier to work with, store that domain in an environment variable:
