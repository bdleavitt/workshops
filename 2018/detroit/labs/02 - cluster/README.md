...
1. In the Azure Portal open the Public IP resource named **dtr_app_ip**. Copy and paste the DNS name into a new browser tab, prepended with `https://`. 

	Log into DTR with **admin** and **DockerEE123!**
...
1. From the [Azure Portal](https://portal.azure.com), navigate to your Resource Group and select the `<RGName>-worker-linux-1`, (ex. `docker-ee-2-worker-linux-1`). 

	Click **Connect** and copy the SSH command to your laptop/workstation. 

	![image](./images/ssh.png)

	Before we can connect, we first need to add an `-i id_rsa.pub` to the command, as the VMs are configured to use a certificate located in the `dci` directory from Lab 1. Navigate to the `dci` folder and run the adjusted ssh command, ex. `ssh -i id_rsa dockeradmin@40.117.112.89` to open a connection to the VM

2. First, let's configure an environment variable for the Docker Trusted Registry URL. Navigate in the Azure Portal to the **dtr_app_ip** resource, and copy its DNS name.

3. Set an environment variable `DTR_HOST` using the DTR host name defined on your Play with Docker landing page:

	```bash
	$ export DTR_HOST=dtrapp-dockeree2-alk7.eastus.cloudapp.azure.com
	$ echo $DTR_HOST
	```
...
1. In the Azure Portal, open the Resource Group and then the Public IP for **ucp_app_ip**. Open the DNS name in a browser, prepended with `https`, and you'll have the same warnings regarding `https` that you have before. Click through those and log in. You'll see the Universal Control Panel dashboard.
...
3. You'll see a `Create Secret` screen. Type `mysqlpassword` in `Name` and `Dockercon!!!` in `Content`. Then click `Create` in the lower left. Obviously you wouldn't use this password in a real production environment. You'll see the content box allows for quite a bit of content, you can actually create structured content here that will be encrypted with the secret.
...
`dtrapp-dockeree2-alk7.eastus.cloudapp.azure.com`
...
8. Click on `Stacks` again, and select the `java_web` stack. On the top nav, select **Services**, and select `java_web_webserver`. In the panel, you'll see `Published Endpoints`. 

	![](./images/javawebwebserver.png)

	> The app is published at the `:8080` at the end. Currently the load balancer rules and Network Security group are limiting traffic to port `:8080`. Either take a moment to adjust their configuration, or you can see the published app in this image:
...
1. From the [Azure Portal](https://portal.azure.com) open the blade for the first windows node, ex. **docker-ee-win-1**, and download the `.rdp` file. 

	Log into the VM with **dockeradmin** and **DockerEE123!**

	Once in the VM, open a PowerShell window as Administrator and see what is running

	```powershell
	docker ps
	```

	The agent containers are communicating with the cluster. For the first few exercises we will work directly on the node, before stepping back and using UCP.

	Start by installing Git, and then cloning the Git repository for the sample app:
	...
	# Do not worry about the OpenSSH note after installation
	scoop install git
...
2. Set an environment variable for the DTR host name. Much like you did for the Java app, this will make a few step easier. Copy the DTR host name again and create the environment variable. For instance, if your DTR host was `dtrapp-dockeree2-alk7.eastus.cloudapp.azure.com` you would type:

	```powershell
	PS C:\> $env:DTR_HOST="dtrapp-dockeree2-alk7.eastus.cloudapp.azure.com"
	```
	
	Additionally, since DTR is using a self-signed certificate we need to tell the Docker Engine that it is OK to push to an otherwise insecure registry. We will update the engine configuration with:

	```powershell
	Set-Content -Path "C:\ProgramData\docker\config\daemon.json" -Value "{`"insecure-registries`" : [ `"$env:DTR_HOST`" ]}" -Force
...
1. First we need to update the Java web app so it'll take advantage of the .NET API. Switch back to your `*-linux-worker-1` VM and change directories to the `java-app-v2` directory. Repeat steps 1,2, and 4 from Task 2.2 but add a tag `:2` to your build and pushes:
...
1. Back in the SSH session to the Linux worker node,  CD into the `hybrid-app/dotnet-api` directory. 
